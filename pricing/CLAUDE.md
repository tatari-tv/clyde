...
- **Publish (`.github/workflows/pages.yml`, repo root):** merging a `pricing/data/pricing.json` change to `main` deploys it to GitHub Pages at the URL above. The workflow stages **two** files: the feed as `/pricing.json`, and `pricing/site/index.html` as the site root `/`.
- **Site root (`pricing/site/index.html`):** a static page documenting the feed, served at `https://tatari-tv.github.io/clyde/`. It reads `./pricing.json` client-side and renders the version fields, the model count, and the rates table from that fetch, so it cannot drift from the feed it documents; a failed fetch renders a visible error rather than an empty table. It is deliberately NOT a dashboard (no charts, no history, no analytics). Source lives in `pricing/site/` because `pricing/data/` holds generated artifacts only.
  - **Editing the page:** `pricing/site/index.html` is in `pages.yml`'s `on.push.paths`, so an index-only change to `main` deploys on its own. A new file under `pricing/site/` needs **BOTH** edits in `pages.yml`, and each omission fails a different way: add it to `on.push.paths` or edits to it never deploy (the workflow simply does not run, and **nothing reports an error**), AND add a `cp` line to the staging step or the deploy fires without the file ever reaching `_site` -- a green deploy serving a 404. The workflow copies `pricing/data/pricing.json` and `pricing/site/index.html` by name; it does not glob the directory.
