...
- **Site root (`pricing/site/index.html`):** a static page documenting the feed, served at `https://tatari-tv.github.io/clyde/`. It reads `./pricing.json` client-side and renders the version fields, the model count, and the rates table from that fetch, so it cannot drift from the feed it documents; a failed fetch renders a visible error rather than an empty table. It is deliberately NOT a dashboard (no charts, no history, no analytics). Source lives in `pricing/site/` because `pricing/data/` holds generated artifacts only.
  - **Editing the page:** `pricing/site/index.html` is in `pages.yml`'s `on.push.paths`, so an index-only change to `main` deploys on its own. If you ever add another file under `pricing/site/`, add it to `paths` too — omitting it means edits never deploy and **nothing reports an error**, because the workflow simply does not run.
