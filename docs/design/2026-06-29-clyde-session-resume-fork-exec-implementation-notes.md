...
### Open questions
- RESIDUAL #5 WINDOW (could not be fully closed at this layer): the checkpoint+rename closes the window for a permit-log process that opens the DB AFTER the rename (it lands on a fresh `events.db`). It does NOT close the window for a permit-log process that ALREADY had the legacy `events.db` open (an existing rusqlite handle / open fd) before the checkpoint+rename: on Linux that handle keeps writing to the same inode (now named `events.db.merging`), and any rows it commits after our `PRAGMA wal_checkpoint(TRUNCATE)` but before it notices the rename land in the staged snapshot's WAL: they are merged on THIS run only if they were checkpointed into the staged main file by our count time, otherwise they ride along in the `.clyde.bak` and are not in the clyde DB. Fully closing this needs cooperation from the writer (e.g. an advisory lock or a "stop writing during migration" signal to the permit hook), which is out of scope for a bootstrap-side hardening pass. In practice the legacy DB stopped being written at cutover, so a live open handle racing the bootstrap is unlikely; the `.clyde.bak` preserves any such stragglers for manual recovery. Tracked here for a follow-up if the permit hook ever writes concurrently with bootstrap in the field. **(SUPERSEDED by the Phase 5 hardening II note below — the checkpoint is now fail-closed and the straggler sidecars are preserved with the backup, shrinking this residual.)**

## Phase 5 hardening II (checkpoint fail-closed)

Final re-audit finding: both `PRAGMA wal_checkpoint(TRUNCATE)` sites discarded their result. SQLite reports a lock-blocked checkpoint as `SQLITE_OK` with `busy=1` (the first column of the returned row), NOT as an error, so a plain `execute_batch` silently treated a blocked checkpoint as success and the code then moved/deleted the `-wal`, potentially stranding committed frames. This pass makes both checkpoints fail-closed and preserves any straggler frames with the backup.

### Design decisions
- Fail-closed checkpoint helper - `clyde/src/bootstrap.rs:checkpoint_truncate` - reads the `busy` column (column 0 of the `(busy, log, checkpointed)` row that `wal_checkpoint(TRUNCATE)` returns) via `query_row` and returns `Err` (with `path` context) when `busy != 0`. Both former `execute_batch("PRAGMA wal_checkpoint(TRUNCATE);")` calls now go through it: the MOVE path in `migrate_events_db` (the `?` propagates BEFORE `fs::rename(legacy -> dest)`, so on a blocked checkpoint nothing is moved and the legacy DB is untouched) and the claim path in `merge_events_db` (the `?` propagates BEFORE `fs::rename(legacy -> staging)` and before any sidecar handling, so on a blocked checkpoint no staging file is created and the legacy DB + its `-wal` are left intact).
- Preserve straggler sidecars with the backup - `clyde/src/bootstrap.rs:merge_events_db` - the claim path no longer DELETES the legacy `-wal`/`-shm` after the claim rename. Instead it MOVES each present sidecar alongside the staging file (`sidecar(legacy, sfx) -> sidecar(&staging, sfx)`), and at finalize moves them alongside the `.clyde.bak` (`sidecar(&staging, sfx) -> sidecar(&bak, sfx)`), so the `.clyde.bak` set is a complete, replayable DB. This preserves any frames a permit-log fd opened BEFORE the claim wrote into the legacy `-wal` in the tiny checkpoint→rename window. A failed sidecar move is NOT fatal (the merged rows are already durable) — `warn!` and continue. The `merge_events_db` doc comment now says sidecars are preserved with the backup, not discarded.
- Tests - `clyde/src/bootstrap/tests.rs` - `events_db_checkpoint_busy_fails_closed_and_leaves_legacy_intact` (a held `BEGIN IMMEDIATE` writer on the legacy DB forces the TRUNCATE checkpoint to report busy; `migrate_events_db` returns `Err`, the legacy DB + its `-wal` still exist, and the dest was never created); `events_db_merge_preserves_straggler_sidecars_with_backup` (a held autocheckpoint-off writer leaves a non-empty legacy `-wal` at claim time; after the merge the legacy `-wal` is gone, the `.clyde.bak-wal` exists alongside the `.clyde.bak`, the WAL rows merged into clyde, and no orphaned staging sidecar is left). All existing events/merge/dry-run/doctor/WAL-survival/recovery/dedup tests still pass (65 clyde tests, was 63).

### Deviations
- None. (The existing `events_db_merges_legacy_into_clyde_when_both_present` test did not assert legacy-`-wal` deletion — its seed connection closes and checkpoints away the `-wal` before the merge runs — so no existing assertion needed to flip from "deleted" to "moved". The new dedicated `events_db_merge_preserves_straggler_sidecars_with_backup` test covers the moved-not-deleted behavior with a held-open WAL writer.)

### Tradeoffs
- `query_row` reading the `busy` column (fail-closed) vs. `execute_batch` (fire-and-forget) - the former is the whole point of the fix: it lets the caller abort before any destructive rename/delete on a blocked checkpoint, at the cost of one extra column read. No downside.
- Busy test uses `BEGIN IMMEDIATE` (write lock) and accepts a stall up to the 5s `busy_timeout` - the production checkpoint connection carries the merge-wide `EVENTS_BUSY_TIMEOUT_MS = 5_000`, and a held write lock makes the TRUNCATE checkpointer wait that full window before returning `busy=1`. The clyde test suite total is therefore ~5s, bounded by this one test (tests run in parallel). This is the "short stall is unavoidable / acceptable" path called out in the task; the alternative (lowering the timeout) would require a production-code seam purely for the test, which the task forbids.

### Open questions
- TIGHTENED RESIDUAL #5: with the now-fail-closed checkpoint, a blocked checkpoint ABORTS the migration (the `?` fires before any rename/delete), so the legacy DB is preserved for retry rather than losing frames — and straggler `-wal`/`-shm` sidecars now ride along in the `.clyde.bak`, making it a complete replayable DB. The ONLY irreducible window left: a permit-log process holding an fd opened BEFORE the claim that commits frames AFTER our successful (non-busy) checkpoint AND after we have already moved its `-wal` — those specific frames may not reach the clyde DB this run. Fully closing it requires an advisory lock in the permit writer (a separate claude-permit change), and the legacy DB is post-cutover so a live racing writer is not expected. This is NOT a recoverability claim that those exact post-move frames are captured — they may not be; it is the honest, now-much-smaller residual.
- VERIFY-FAILURE TEST DECISION: NOT added as a black-box test, and NO production seam was added. The post-merge verify COUNT (`merge_events_db` Step 4) errors only if reopening `dest` read-only or counting its `events` table fails; by that point `dest` is a valid DB with the `events` table (the merge just wrote to it), so the error path cannot be triggered black-box without injecting a fault via a production-code seam, which the task forbids. That path is covered by code inspection (the `.context("post-merge verification failed; keeping staging snapshot for retry")?` keeps staging and returns `Err` on any COUNT error) plus the busy-checkpoint fail-closed test, which exercises the same `?`-before-destructive-step structure.
