...
- Removed `EnvironmentFile=%h/.config/clyde/enrich.env` from the generated unit body in `clyde/src/bootstrap.rs::install_clyde_timer`.
- **`rewrite_unit` (`clyde/src/bootstrap.rs::rewrite_unit`) changed shape from two chained `.replace()` calls to a line-filter pass**, per the task's explicit instruction: it now (1) applies the existing `klod`->`clyde` and `sessions enrich`->`session enrich` substrings, (2) drops any line whose trimmed start is `EnvironmentFile=`, and (3) optionally injects one `Environment=PATH=...` line immediately before a line starting with `ExecStart=`. `EnvironmentFile=-` (the optional-file form) was deliberately NOT used, per the anti-scope instruction: the line is stripped, not softened.
- **`refresh_clyde_unit` (`clyde/src/bootstrap.rs::refresh_clyde_unit`) trigger broadened from a single `text.contains("sessions enrich")` check to `has_stale_subcommand || has_environment_file`**, where `has_environment_file` scans for any line starting with `EnvironmentFile=`. This is the exact fix the task named as CRITICAL: the live desk.lan unit is already on `session enrich` (singular), so the old trigger would have no-op'd forever and left the `EnvironmentFile` line in place. `backup(svc)` (`clyde/src/bootstrap.rs::backup`) is called before every rewrite, unchanged.
- **G7/R7 (`Environment=PATH=` injection).** Added `resolve_claude_path_env` (`clyde/src/bootstrap.rs::resolve_claude_path_env`), which resolves `claude` via `which::which` at install/repoint time, takes the **parent directory of the resolved path** (not a canonicalized/versioned target -- `which::which` returns the PATH-search hit itself, never resolved through the symlink), and composes it with bootstrap's own inherited `PATH` env var via a pure helper `compose_path_env` (`clyde/src/bootstrap.rs::compose_path_env`): `<claude_dir>:<inherited_path>`, prepend-only. A single-source-of-truth renderer, `environment_path_line` (`clyde/src/bootstrap.rs::environment_path_line`), produces the literal `Environment=PATH=...\n` directive (or `""` when resolution failed), used identically by `install_clyde_timer` (fresh unit) and `rewrite_unit` (existing unit) so the two paths cannot drift.
  - `resolve_claude_path_env` is called once per `repoint_systemd`/`refresh_clyde_unit`/`install_clyde_timer` invocation and threaded down as an explicit `Option<&str>` parameter, rather than called from inside `rewrite_unit` itself. This keeps `rewrite_unit` a pure, directly-testable function (no real `which::which` dependency in its own tests) while the impure resolution step lives in one place, mirroring the existing `Systemd` port pattern's spirit (impure shell-outs kept out of the hermetic core) without introducing a new trait for a single call site.
  - The `.timer` unit rewrite in `repoint_systemd` passes `None` explicitly for `claude_path_env`, since a timer has no `ExecStart=` line and the injection would always be a no-op there; documented inline so a reader does not wonder why the timer branch skips resolution.
  - `rewrite_unit` treats a unit that already carries its own `Environment=PATH=` line as already-fixed and does not inject a second one (idempotent across re-runs, and does not clobber an operator's own override).
- **G6 stale-file warning.** Added `check_stale_env_file` (`clyde/src/bootstrap.rs::check_stale_env_file`), a read-only check for `~/.config/clyde/enrich.env` that returns `Option<PathBuf>` and calls `warn!` when the file exists; it never deletes it. Wired into `bootstrap()` as an unconditional post-migration step (item 3, after data/config migration and integration repointing) that runs identically under `--dry-run` and live, since it performs no mutation and there is nothing to gate. The result is carried on a new `Outcome.stale_env_file: Option<PathBuf>` field, and `run_paths` (`clyde/src/bootstrap.rs::run_paths`) prints an operator-visible line naming the full path via a shared helper `print_stale_env_file_warning`, used by both the dry-run and live output branches so the wording cannot drift between them.
- Deleted `move_env_file` (`clyde/src/bootstrap.rs`) and its call site in `repoint_systemd`'s oneshot-service block. Dropped the "move the API-key env file" phrase from `repoint_systemd`'s doc comment, its inline comments (the oneshot-service comment and the dry-run planning comment), and the `Outcome`/dry-run reporting text.
- **Deleted the three bootstrap tests that wrote a fake key file**, at the exact lines the design doc cited (`clyde/src/bootstrap/tests.rs:56,455,803` under the pre-Phase-5 tree):
  - `seed_full_legacy_world`'s `fs::write(&env_legacy, "ANTHROPIC_API_KEY=secret\n")` line (a shared fixture helper, not a standalone test) was removed; the three `run_*` tests that use it (`run_dry_run_does_not_shell_out_to_systemctl`, `run_live_shells_out_to_systemctl_when_systemd_changed`, `run_skip_systemd_does_not_shell_out`) assert on systemctl-shell-out counts and file existence, neither of which referenced the env file, so removing the line changed no assertion.
  - `systemd_unit_rewrite_moves_env_file_with_perms`, whose entire purpose was verifying `move_env_file`'s permission-preserving move, was deleted outright (that behavior no longer exists) and replaced by `repoint_systemd_strips_environment_file_from_legacy_unit`, which asserts the directive is gone rather than relocated.
  - The env-file-planting lines in `dry_run_performs_zero_mutations_and_lists_planned_steps` were removed; the test's broader dry-run coverage (migrate_dir, events DB, hooks, statusline, pricing, systemd) is otherwise untouched.
- **Replaced by the design-specified "one test asserting no EnvironmentFile line"**: `install_clyde_timer_writes_no_environment_file` (`clyde/src/bootstrap/tests.rs`), which calls `install_clyde_timer` directly and asserts the generated unit body contains no line starting with `EnvironmentFile=`. Additional tests were added beyond that single one because the new line-filtering/injection logic in `rewrite_unit` needed direct, `which`-independent coverage: `compose_path_env_prepends_claude_dir_to_inherited_path`, `rewrite_unit_strips_environment_file_and_injects_claude_path_before_exec_start`, `rewrite_unit_writes_no_environment_line_when_claude_path_env_is_none`, `rewrite_unit_does_not_duplicate_an_existing_environment_path_line`.
- **Existing tests updated for the broadened `refresh_clyde_unit` trigger and the stripped directive**: `repoint_is_noop_for_already_correct_clyde_unit`'s fixture dropped its `EnvironmentFile=` line (a truly-already-correct Phase-5 unit carries neither the stale subcommand nor the directive); `repoint_rewrites_clyde_unit_with_stale_subcommand_and_no_legacy` gained an assertion that `EnvironmentFile` is also stripped in the same rewrite; a new test `repoint_rewrites_clyde_unit_that_still_carries_environment_file` covers the exact live desk.lan case (correct subcommand, stale directive); `systemd_repoints_service_timer_and_enable_symlink` gained an assertion on the rewritten service body.
- New tests `stale_env_file_is_warned_but_never_deleted` and `no_stale_env_file_reports_none` cover both halves of the G6 success criterion against a temp XDG config dir (never the real `~/.config`).

### Deviations

- **`resolve_claude_path_env`/`compose_path_env`/`environment_path_line` are not named anywhere in the design doc's API surface**, which only says "resolve `claude` at install time and write it into the unit." Same effect, correct seam: the doc describes the outcome, not the function decomposition. Split into three functions (impure resolution, pure composition, single-source-of-truth rendering) specifically so the composition and the unit-rewriting logic are unit-testable without depending on the test machine's real `PATH`/`claude` install -- the same reasoning the file already applies to `SystemctlCli`/`Systemd` for the two `systemctl` shell-outs.
- **The `Environment=PATH=` injection was extended to `refresh_clyde_unit` and the full legacy `repoint_systemd` path, not just `install_clyde_timer`.** The task's scope bullet is titled "Resolve `claude` explicitly in the unit" without saying which of the three unit-writing call sites it covers; the design doc's own Risk R7 and the live desk.lan host state (an EXISTING clyde unit that Phase 5 rewrites via the broadened `refresh_clyde_unit` trigger, not a fresh install) make it clear the fix has no effect if it were scoped to `install_clyde_timer` alone -- that function only runs under `--install-timer` on a host with no clyde unit yet, which desk.lan is not. Applying it uniformly to every unit-writing/rewriting path was the only way to make G7 actually reach the live host.
- **`Outcome` gained a new field (`stale_env_file: Option<PathBuf>`) not named in the design's API surface.** The design says bootstrap "emits a `warn!` plus an operator-visible line naming the full path." A `warn!` alone is not independently testable without a log-capture harness (which the rest of this file's tests do not use -- they assert on `Outcome`/filesystem state, e.g. `Outcome.completed`, `Outcome.failed`, `Outcome.systemd_changed`), so the path was also carried on the returned `Outcome` for the same reason those other outcomes are: an assertable, hermetic signal. `run_paths` still separately prints the operator-visible line, satisfying "operator-visible" as stated.

### Tradeoffs

- **`resolve_claude_path_env`, `daemon_reload`, and `start_enrich_timer` are the only three functions in this file that shell out to or query real host state, and only the latter two were already routed through the `Systemd` port for testability.** `resolve_claude_path_env` was NOT given an equivalent injected-fake port. Chosen because (a) it is a pure read (`which::which` + `env::var`), not a mutation, so there is no "did it happen" assertion to make the way `CountingSystemd` proves `daemon_reload`/`start_enrich_timer` were or weren't called; (b) its actual output value (a composed PATH string) is exactly what `compose_path_env` isolates for direct unit testing; a fake `ClaudeResolver` trait would let a test assert the same thing with strictly more code and one more generic parameter threaded through `repoint_systemd`/`refresh_clyde_unit`/`install_clyde_timer`. Revisit if a future phase needs to assert end-to-end "given this fake PATH state, the final written unit says X" without the split.
- **`rewrite_unit`'s `Environment=PATH=` injection point is "immediately before the first `ExecStart=` line" rather than a fixed position (e.g. always right after `[Service]`).** Chosen because it makes the function robust to whatever the surrounding unit body looks like (a hand-edited unit, a future template reordering) without parsing INI sections, and it naturally makes the injection a no-op on a `.timer` file (which has no `ExecStart=`) without a separate "is this a service or a timer" parameter -- the file shape itself decides.
- **The G6 warning is unconditional (no `--force`/opt-out flag) and runs on every `bootstrap` invocation, including ones that migrate nothing else.** Considered gating it behind existing flags (e.g. skip when `--skip-systemd`), but the stale file is not a systemd concern by the time this runs -- it is a leftover credential regardless of which other steps ran -- so gating it on an unrelated flag would silently suppress the one signal the design says must survive (Non-Goals: "What clyde owes instead is not leaving it to rot silently").

### Open questions

- None blocking. Two items are explicitly PENDING-ORCHESTRATOR, per the task's anti-scope instruction, and were not run here:
  - The live desk.lan criterion (`systemctl --user start clyde-enrich.service` after a reinstall enriches a session) requires a real install and mutates the live db and the live systemd unit.
  - Confirming that `resolve_claude_path_env` actually resolves `claude` on desk.lan and writes a working `Environment=PATH=` line requires running the real `bootstrap` binary there, not the hermetic test suite.
