...
- G6 (Scott, 2026-07-29): no `.env` file, anywhere, for any purpose. `taste.md` says "env vars never .env". clyde stops generating one, stops migrating one, and stops reading one, and it does not gain a replacement config key to carry a credential value. Nothing needs delivering.
...
- [ ] AC1: no Rust file in the workspace **reads** `ANTHROPIC_API_KEY` or sends `x-api-key`. Mechanically, BOTH halves must hold:
...
- [ ] AC2: on a keyless host with a logged-in `claude`, all three LLM commands exit 0 and produce model-authored output: `env -u ANTHROPIC_API_KEY clyde session enrich --only <id>` (row gains tags + summary + non-zero token counts), `... clyde efficiency session <id> --narrate` (prose verdict), `... clyde report render -i <real>.json --format markdown -o -` (>5,000 bytes, >=3 `^## ` headers, no `Generated offline via`).
- [ ] AC3: there is exactly one **production** `Transport` implementation in the workspace: `rg 'impl Transport for' --glob '*.rs' --glob '!**/tests.rs' --glob '!target/**'` returns one hit, in `common/src/llm/cli.rs`. Test doubles are excluded deliberately and must NOT be deleted to satisfy this. Measured today: five impls exist (`summarize/api.rs:70`, `summarize/cli.rs:125`, `eval/judge/tests.rs:33` `Canned`, `render/slots/tests.rs:53` `Scripted`, `:74` `Exploding`); Phase 4 removes exactly one of them, the api transport, leaving one production impl and three doubles.
- [ ] AC4 (G6): the systemd unit `install_clyde_timer` generates contains no `EnvironmentFile` line, `move_env_file` no longer exists in the tree, and no code path generates, moves, or reads a `.env` file. The only surviving mention of `enrich.env` in Rust is the stale-file warning.
- [ ] AC5: with `claude` absent from PATH, every one of the three commands fails non-zero with an error naming the install-and-login remedy, and none of them mentions `ANTHROPIC_API_KEY`.
...
- [ ] AC7 (G5): a sweep against an unavailable `claude` charges `attempts` to at most the rows the breaker observed failing, never to the whole candidate set, and exits non-zero. Mechanically, against a seeded db of >= 20 candidates:
