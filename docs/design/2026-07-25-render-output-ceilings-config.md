...
- [x] AC-C3: the ceiling is no longer a compile-time fact. `rg 'fn max_output_tokens\(|fn api_limits' report/src/` returns nothing, and `Transport::complete` takes four arguments. **The pattern is anchored on the open paren deliberately:** the bare form `fn max_output_tokens` also matches `Kind::max_output_tokens_key` (`report/src/summarize.rs:38`), which Phase 2 ADDED and which returns a config-key name, not a ceiling. Anchoring keeps the gate testing what it means -- that no method hands back an output ceiling.
...
- [x] AC-C6: no Guard 6 test passes vacuously. `guard_output_ceiling_allows_the_same_output_for_the_larger_job` uses an output value strictly between the two ceilings, proven by flipping its `Kind::Markdown` assertion to `is_ok()` and watching it fail (the variant was `Job::Markdown` when this AC was written; Phase 1 moved it to `Kind`).
