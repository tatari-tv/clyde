...
- **Both `compute_cache_key` sites, not one.** `cost/src/lib.rs:283` is the single-day READ; `cost/src/lib.rs:489` is the multi-day WRITE. Fixing only the read side leaves the write side stamping `"none"` while the read side looks for a version, so every multi-day-written entry misses forever.
...
- [x] AC-P8: `Pricing::embedded().data_version()` equals the `data_version` in `pricing/data/pricing.json`, and two embedded baselines with different versions produce different `compute_cache_key` values at **both** call sites (`cost/src/lib.rs:283` read, `:489` write). Today both collapse to the `"none"` bucket.
